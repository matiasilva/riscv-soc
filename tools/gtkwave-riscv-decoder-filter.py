...
def main():
    ...
    while True:
        ...
        with tempfile.NamedTemporaryFile(delete=False, mode='w') as asm_temp:
            ...
            asm_temp.flush()
            subprocess.run(["riscv64-unknown-elf-as", "-mno-arch-attr", "-o", obj_temp.name, asm_temp.name])
            result = subprocess.run(["riscv64-unknown-elf-objdump", "-D", "-b", "elf32-littleriscv", "--disassembler-options=numeric,no-aliases", obj_temp.name], capture_output=True)
